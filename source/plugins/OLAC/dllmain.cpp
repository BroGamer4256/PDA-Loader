...
// MLAC Created by Samyuu in 2019
// OLAC, slightly modified version of MLAC, created by Jay39w in 2020
// Please credit Samyuu for any/all code used here within

using namespace OLAC::Components;
using namespace OLAC::Utilities;
using namespace OLAC::FileSystem;
...
namespace OLAC
{
	...
	void InitializeTick()
	{
		...
		if ((MessageThread.Handle = CreateThread(0, 0, WindowMessageDispatcher, 0, 0, 0)) == NULL)
			printf("[OLAC] InitializeTick(): CreateThread() Error: %d\n", GetLastError());
		...
		if (DeviceConnected)
		{
			...
			if (!Input::DualShock4::InstanceInitialized())
			{
				if (Input::DualShock4::TryInitializeInstance())
					printf("[OLAC] UpdateTick(): DualShock4 connected and initialized\n");
			}
		}
		...
		if (HasWindowFocus)
		{
			...
			if (Input::DualShock4::GetInstance() != nullptr)
			{
				if (!Input::DualShock4::GetInstance()->PollInput())
				{
					...
					printf("[OLAC] UpdateTick(): DualShock4 connection lost\n");
				}
			}
...
BOOL APIENTRY DllMain(HMODULE hModule, DWORD  ul_reason_for_call, LPVOID lpReserved)
{
	...
	case DLL_PROCESS_ATTACH:
		printf("[OLAC Notice] MLAC was created by samyuu in 2019, please give any credit for OLAC to him! \n");
		if (*(char*)0x005DB6C4 == (char)0x65 || *(char*)0x005DB6C4 == (char)0x00)
		{
		}
		else
		{
			printf("[OLAC] Version does not match 1.01, continuing anyway... \n");
		}
		printf("[OLAC] Installing hooks...\n");

		OLAC::InitializeHooks();
		OLAC::MainModule::Module = hModule;
...
LRESULT CALLBACK MessageWindowProcessCallback(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_CREATE:
	{
		...
		if (!RegisterDeviceInterface(hWnd, &hDevNotify))
			printf("[OLAC] MessageWindowProcessCallback(): RegisterDeviceInterface() Error: %d\n", GetLastError());
...
VOID RegisterMessageWindowClass()
{
	...
	windowClass.hInstance = OLAC::MainModule::Module;
